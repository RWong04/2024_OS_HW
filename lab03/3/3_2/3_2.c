...
void *thread1(void *arg){
    ...
    fclose(fptr4);
    fptr4 = fopen("/proc/Mythread_info", "w");  // 打開文件進行寫入
    if (fptr4 != NULL) {
        fwrite(data, 1, strlen(data), fptr4);
        fclose(fptr4);  // 寫入後關閉文件
    } else {
        printf("Failed to open /proc/Mythread_info for writing.\n");
    }
...
void *thread2(void *arg){
    ...
    /* Hint: Write data into proc file.*/
    fclose(fptr5);
    fptr5 = fopen("/proc/Mythread_info", "w");  // 打開文件進行寫入
    if (fptr5 != NULL) {
        fwrite(data, 1, strlen(data), fptr5);
        fclose(fptr5);  // 寫入後關閉文件
    } else {
        printf("Failed to open /proc/Mythread_info for writing.\n");
    }
